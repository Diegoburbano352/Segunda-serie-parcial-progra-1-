...
void buscar_indice(){
	...
	int pos=0;
	cout<<"Que registro desea visualizar: ";
	cin>>pos;
	cout<<"------------------ "<<pos<<" ------------------"<<endl;
	fseek ( archivo,pos * sizeof(Empleado), SEEK_SET );
...
void modificar_empleado(){
	...
	FILE* archivo = fopen(nombe_archivo, "r+b");
		...
		int id;
		string nombre,apellido;	
		char sueldo,bonificacion,puesto;
    	Empleado empleado;
...
void eliminar(){
      int pos = 0;
      cout<<"\n Ingrese el id a eliminar: "<<endl;
      cin>>pos;
      
      FILE* archivo = fopen(nombe_archivo, "r+b");
      FILE* archivo2 = fopen("auxis.dat", "a+b");
      Empleado empleado;
      fread ( &empleado, sizeof(Empleado), 1, archivo );
      
      while (feof( archivo ) == 0){
      	if (pos == empleado.codigo){
      		cout<<"\n El registro ah sido eliminado correctamente "<<endl;
      		cout<<endl;
		}else 
